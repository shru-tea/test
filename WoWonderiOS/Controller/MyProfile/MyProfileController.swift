...
//        self.userData.updateValue(AppInstance.instance.profile?.userData?.avatar ?? "", forKey: "avatar")
        self.userData = ["name":UserData.getUSER_NAME(),"wallet":UserData.getWallet(),"is_pro":UserData.getIsPro(),"avatar":UserData.getImage(),"first_name":AppInstance.instance.profile?.userData?.firstName ?? "","last_name":AppInstance.instance.profile?.userData?.lastName ?? "","phone_number":AppInstance.instance.profile?.userData?.phoneNumber ?? "","address":AppInstance.instance.profile?.userData?.address ?? "","working":AppInstance.instance.profile?.userData?.working ?? "","school":AppInstance.instance.profile?.userData?.school ?? "","website":AppInstance.instance.profile?.userData?.website]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "MyProfile") as! MyProfileCell
            ...
            if let about = userData["about"] as? String{
                if about == ""{
                    cell.statusTextLbl.text = "Hi there i am using Wowonder Timeline"
                }
                else{
            ...
            self.tableView.rowHeight = 323.0
            return cell
        }
...
//            cell.photoBtn.addTarget(self, action: #selector(self.GotoAddPost(sender:)), for: .touchUpInside)
    ...
    func editProfile(firstName: String, lastName: String, phone: String, webSite: String, WorkPlace: String, School: String, location: String) {
        self.userData["name"] = "\(firstName)\(" ")\(lastName)"
