...
class GetUserListManager{
    ...
    var result3:GetUserListModel.TwilloAudioCallData?
    ...
    func getUserList(user_id: String, session_Token: String, completionBlock: @escaping (_ Success:GetUserListModel.GetUserListSuccessModel?,_ RoomName:String?,_ Call_Id:String?,_ senderName:String?,_ profileImage:String?,_ type:String?,_ accessToken2:String?,_ AuthError:GetUserListModel.GetUserListErrorModel?,_ ServerKeyError:GetUserListModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        Alamofire.request(API.GetUserList_Constants_Methods.GET_USER_LIST_API, method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            print(params)
            if (response.result.value != nil){
                ...
                if apiStatus is Int{
                    ...
                    if twilloAudioCallData is [Any]{
                        ...
                        self.result3 = try? JSONDecoder().decode(GetUserListModel.TwilloAudioCallData.self, from: data1!)
                    ...
                    if self.result1 != nil{
                        ...
                        completionBlock(result,self.result2?.data?.roomName,self.result2?.data?.id,self.result2?.name,(self.result2?.avatar)!, "video",self.result2?.data?.accessToken2,nil, nil, nil)
                        
                    }else if self.result3 != nil{
                        completionBlock(result,self.result3?.data?.roomName,self.result3?.data?.id,self.result3?.name,self.result3?.avatar, "audio",self.result2?.data?.accessToken2,nil, nil, nil)
                    }else{
