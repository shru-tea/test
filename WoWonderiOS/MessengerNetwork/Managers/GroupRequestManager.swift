...
class GroupRequestManager{
    ...
    func fetchGroupRequest(session_Token: String,fetchType:String,offset:Int,setOnline:Int,completionBlock: @escaping (_ Success:GroupRequestModel.GroupRequestSuccessModel?,_ AuthError:GroupRequestModel.GroupRequestErrorModel?,_ ServerKeyError:UserSuggestionModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        Alamofire.request(API.GROUP_REQUEST_API.GROUP_REQUEST_API + "\(session_Token)", method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
    ...
    func accceptGroupRequest(session_Token: String,type:String,groupID:Int,completionBlock: @escaping (_ Success:AcceptGroupRequestModel.AcceptGroupRequestSuccessModel?,_ AuthError:AcceptGroupRequestModel.AcceptGroupRequestErrorModel?,_ ServerKeyError:AcceptGroupRequestModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        let decoded = String(data: jsonData, encoding: .utf8)!
        log.verbose("Decoded String = \(decoded)")
        Alamofire.request(API.GROUP_REQUEST_API.ACCEPT_GROUP_REQUEST_API + "\(session_Token)", method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            if (response.result.value != nil){
                guard let res = response.result.value as? [String:Any] else {return}
                log.verbose("Response = \(res)")
                guard let apiStatus = res["api_status"]  as? Any else {return}
    ...
    func rejectGroupRequest(session_Token: String,type:String,groupID:Int,completionBlock: @escaping (_ Success:RejectGroupRequestModel.RejectGroupRequestSuccessModel?,_ AuthError:RejectGroupRequestModel.RejectGroupRequestErrorModel?,_ ServerKeyError:RejectGroupRequestModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        let decoded = String(data: jsonData, encoding: .utf8)!
        log.verbose("Decoded String = \(decoded)")
        Alamofire.request(API.GROUP_REQUEST_API.REJECT_GROUP_REQUEST_API + "\(session_Token)", method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            if (response.result.value != nil){
                guard let res = response.result.value as? [String:Any] else {return}
                log.verbose("Response = \(res)")
                guard let apiStatus = res["api_status"]  as? Any else {return}
