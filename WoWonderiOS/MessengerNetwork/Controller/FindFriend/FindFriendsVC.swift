...
//        }
    ...
    private func fetchData(lat:Double,long:Double,genderString:String,status:Int,distance:Int){
        ...
        Async.background({
            MessengerFIndFriendManager.instance.fetchFindFriends(session_Token: sessionToken, gender: genderString, status: status, distance: distance, lat:lat, long: long, limit: 20, completionBlock: { (success, sessionError, serverError, error) in
                if success != nil{
                    Async.main({
