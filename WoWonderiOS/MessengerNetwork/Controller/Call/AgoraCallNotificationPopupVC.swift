...
@objc func dismissView(sender: UITapGestureRecognizer) {
                        ...
                        if success != nil{
                            Async.main({
                                self.dismissProgressDialog {
                                    ...
                                    self.checkForCallAction(callID: self.callId!, callingStatus: self.callingStyle!, accessToken: (success?.accessToken)!)
                                    self.timer = Timer.scheduledTimer(timeInterval: 0.6, target: self, selector: #selector(self.update), userInfo: nil, repeats: true)
                                }
                            })
                        }else if sessionError != nil{
    ...
    @objc func update() {
        self.checkForCallAction(callID: self.callId!, callingStatus: callingStyle!, accessToken: "")
